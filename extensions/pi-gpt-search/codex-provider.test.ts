...
const stubCtx = { modelRegistry: { getProviderAuth: async () => ({ auth: { apiKey: "token" } }) } } as never;
...
test("recordRefs accumulates ref metadata across responses within one session", async () => {
	...
	await provider.execute({ search_query: [{ q: "a" }] }, stubCtx);
	await provider.execute({ open: [{ ref_id: "turn0search0" }] }, stubCtx);
...
test("recordRefs ignores results without ref_id or url", async () => {
	...
	await provider.execute({ search_query: [{ q: "c" }] }, stubCtx);
...
test("execute sends session id, model, and serialized commands in the payload", async () => {
	...
	await provider.execute({ find: [{ ref_id: "turn0search0", pattern: "x" }] }, stubCtx);
...
test("open and find fail fast with guidance when a reference is stale", async () => {
	...
	await assert.rejects(
		provider.execute({ open: [{ ref_id: "turn9search9" }] }, stubCtx),
		/Unknown or stale reference "turn9search9"/u,
	);
	await assert.rejects(
		provider.execute({ find: [{ ref_id: "turn9view0", pattern: "x" }] }, stubCtx),
		/Unknown or stale reference "turn9view0"/u,
	);

	const indexed = providerWithResponses([{ results: [{ ref_id: "turn0search0", url: "https://a.io" }] }]);
	await indexed.provider.execute({ search_query: [{ q: "a" }] }, stubCtx);
	const response = await indexed.provider.execute({ open: [{ ref_id: "turn0search0" }] }, stubCtx);
...
test("stale reference errors list the known ref ranges for self-correction", async () => {
	...
	await indexed.provider.execute({ search_query: [{ q: "a" }] }, stubCtx);
	await indexed.provider.execute({ search_query: [{ q: "b" }] }, stubCtx);
	await assert.rejects(
		indexed.provider.execute({ open: [{ ref_id: "turn0search29" }] }, stubCtx),
