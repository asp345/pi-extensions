import { abortIf, splitLines } from "../utils.ts";
import { defaultHashIdentity, HASH_SEP } from "./hash-identity.ts";
import {
	type AutoFix,
	type BDup,
...
import { AnchorMismatchError, type ResolvedRange, verifyServedRange } from "./served.ts";
...
export function changedRange(
	...
	while (lastOrig >= first && lastRes >= first && originalLines[lastOrig] === resultLines[lastRes]) {
		lastOrig--;
		lastRes--;
	}
	return {
