...
import { detectEnding, type LineEnding, stripBOM, toLF } from "./edit-diff.ts";
import { type LFile, loadFileKindAndText } from "./file-kind.ts";
import { resolveTarget } from "./fs-write.ts";
import { type HashStore, loadHashStore } from "./hash-store.ts";
import { defaultHashIdentity } from "./hashline/hash-identity.ts";
...
export async function readNormFile(path: string, cwd: string, options?: ReadNormOptions): Promise<NormFile> {
