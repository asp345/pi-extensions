...
async function loginWithManualApiKey(callbacks: OAuthLoginCallbacks, reason?: string): Promise<OAuthCredentials> {
...
async function loginWithWorkosCredentials(
	...
	if (clineAuth.expiresAt <= Date.now() + WORKOS_REFRESH_MARGIN_MS) {
		const tempCred = credentialsFromWorkos(clineAuth.accessToken, clineAuth.refreshToken, clineAuth.expiresAt);
