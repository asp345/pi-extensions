 ...
 * @internal
 */
export function credentialsFromWorkos(accessToken: string, rtToken: string, expiresAt: number): OAuthCredentials {
	return {
		access: accessToken,
		refresh: rtToken,
		expires: expiresAt,
...
export async function refreshWorkosToken(
	...
	try {
		...
		if (err instanceof DOMException && err.name === "AbortError") {
			throw new Error("ClinePass token refresh timed out — check your network or try a static API key.", {
				cause: err,
			});
		}
		throw err;
...
function extractWorkosAuthFromRecord(auth: Record<string, unknown>): ClineAuthCredentials | undefined {
 ...
 */
export function resolveClineAuthCredentials(options: AuthKeyOptions = {}): ClineAuthCredentials | undefined {
