...
function toRoutingModel(value: unknown): RoutingModel | undefined {
	const model = value as Partial<RoutingModel> | null | undefined;
...
function asPiModel(model: RoutingModel): Model<Api> {
	return {
		provider: model.provider,
		id: model.id,
		name: model.id,
		...
		thinkingLevelMap: model.thinkingLevelMap,
		input: model.input?.filter((entry): entry is "text" | "image" => entry === "text" || entry === "image") ?? ["text"],
