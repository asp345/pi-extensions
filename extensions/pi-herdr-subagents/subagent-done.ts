...
export function shouldAutoExitOnAgentEnd(
	...
	messages: readonly { role?: string; stopReason?: string; errorMessage?: unknown }[] | undefined,
): boolean {
...
export function buildCompletionSidecar(
	messages: readonly { role?: string; stopReason?: string; errorMessage?: unknown }[] | undefined,
): { type: "done" } | { type: "error"; errorMessage: string; stopReason: "error" } {
...
export default function (pi: ExtensionAPI) {
		...
		ctx.ui.setWidget(
			...
			(_tui, theme) => {
				...
				if (expanded) {
					...
					let deniedLine = "";
					if (denied.length > 0) {
						const deniedList = denied.map((name: string) => theme.fg("error", name)).join(theme.fg("muted", ", "));
	...
	pi.on("agent_end", (event, ctx) => {
		...
		if (shouldExit) {
			...
			if (sessionFile) {
				try {
					writeFileSync(`${sessionFile}.exit`, JSON.stringify(buildCompletionSidecar(messages)));
				} catch {
