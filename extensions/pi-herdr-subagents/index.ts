...
const globalStore = globalThis as unknown as {
	...
	[RUNTIME_KEY]?: SubagentRuntime;
};

{
	const prevInterval = globalStore[WIDGET_INTERVAL_KEY];
	if (prevInterval) {
		clearInterval(prevInterval);
		globalStore[WIDGET_INTERVAL_KEY] = null;
	}
	const prevStatusInterval = globalStore[STATUS_INTERVAL_KEY];
	if (prevStatusInterval) {
		clearInterval(prevStatusInterval);
		globalStore[STATUS_INTERVAL_KEY] = null;
	}
}
 ...
 */
function resolveEffectiveAutoExit(params: Static<typeof SubagentParams>, agentDefs: AgentDefaults | null): boolean {
...
function resolveEffectiveInteractive(params: Static<typeof SubagentParams>, agentDefs: AgentDefaults | null): boolean {
...
function loadAgentDefaults(agentName: string): AgentDefaults | null {
	...
	const paths = [join(process.cwd(), ".pi", "agents", `${agentName}.md`), join(configDir, "agents", `${agentName}.md`)];
...
const runtime: SubagentRuntime = (globalStore[RUNTIME_KEY] ??= createSubagentRuntime());
...
function formatLifecycleWidgetLabel(projection: ReturnType<typeof projectLifecycle>, now: number): string {
	const duration =
		projection.stateDurationSince == null ? "" : ` ${formatElapsedDuration(now - projection.stateDurationSince)}`;
	if (projection.kind === "active")
		return projection.label ? ` active · ${projection.label}${duration} ` : ` active${duration} `;
...
function renderSubagentWidgetLines(agents: RunningSubagent[], width: number): string[] {
	...
	for (const { agent, projection } of rendered) {
		...
		const runtimeTag = agent.runtimePlan ? `${agent.runtimePlan.modelId}|${agent.runtimePlan.thinking} · ` : "";
		...
		lifecycle = observeActivity(
			lifecycle,
			{
				...
				activity: {
					...
					...(state.activeScope ? { activeScope: state.activeScope as SubagentActivityScope } : {}),
...
function requestSubagentInterrupt(
	...
	try {
		...
		return {
			error: `Failed to send Escape to subagent "${running.name}" via herdr: ${message}`,
		};
	}
}
...
function startStatusRefresh(pi: ExtensionAPI) {
	...
	globalStore[STATUS_INTERVAL_KEY] = statusInterval;
}
...
function startWidgetRefresh() {
	...
	globalStore[WIDGET_INTERVAL_KEY] = widgetInterval;
}
...
async function watchSubagent(running: RunningSubagent, signal: AbortSignal): Promise<SubagentResult> {
	...
	try {
		...
		running.lifecycle = markFailed(
			running.lifecycle,
			signal.aborted ? "Subagent cancelled." : err instanceof Error ? err.message : String(err),
			Date.now(),
			1,
		);
		updateWidget();
		...
		return {
			name,
			task,
			summary: `Subagent error: ${errorText}`,
			exitCode: 1,
			elapsed: Math.floor((Date.now() - startTime) / 1000),
			error: errorText,
		};
...
export default function subagentsExtension(pi: ExtensionAPI) {
	...
	if (shouldRegister("subagent"))
		pi.registerTool({
			...
			renderCall(args, theme) {
				...
				const agent =
					typeof partialArgs.agent === "string" && partialArgs.agent ? theme.fg("dim", ` (${partialArgs.agent})`) : "";
				const cwdHint =
					typeof partialArgs.cwd === "string" && partialArgs.cwd ? theme.fg("dim", ` in ${partialArgs.cwd}`) : "";
			...
			renderResult(result, _opts, theme) {
				...
				if (details?.status === "started") {
					const runtime = details?.model
						...
						: " — started";
					return new Text(
						theme.fg("accent", "▸") + " " + theme.fg("toolTitle", theme.bold(name)) + theme.fg("dim", runtime),
						0,
						0,
					);
				}
	...
	if (shouldRegister("subagent_resume"))
		pi.registerTool({
			...
			renderResult(result, _opts, theme) {
				...
				if (details?.status === "started") {
					return new Text(
						theme.fg("accent", "▸") + " " + theme.fg("toolTitle", theme.bold(name)) + theme.fg("dim", " — resumed"),
						0,
						0,
					);
				}
	...
	pi.registerMessageRenderer("subagent_result", (message, options, theme) => {
		const details = message.details as RenderedDetails | undefined;
