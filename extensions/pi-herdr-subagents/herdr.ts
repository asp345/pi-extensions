...
export function createHerdrSurfaceSplit(name: string, direction: "right" | "down"): string {
...
function parsePaneGetError(error: unknown): PaneInspectionResult {
	const failure = (error ?? {}) as { stderr?: unknown; stdout?: unknown; message?: unknown };
	for (const raw of [failure.stderr, failure.stdout]) {
	...
	const message = failure.message ? String(failure.message) : "herdr pane get failed";
