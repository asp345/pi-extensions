...
Native compaction applies only to models reporting `capabilities.compaction` on the Models API. The result is cached per model for the process lifetime; a failed Models API request is not cached. Models without that capability use Pi prompt-based text compaction. A failed Models API request or summary request (including a `refusal` stop reason) cancels compaction and shows the error, as on the Codex path.
