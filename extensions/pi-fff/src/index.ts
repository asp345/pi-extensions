...
export { SCAN_TIMEOUT_MS } from "./sdk.ts";
...
function formatFindOutput(result: SearchResult, limit: number, pattern: string): FormattedFind {
	...
	const reordered: Array<{ item: FileItem }> = result.items.map((item: FileItem) => ({ item }));
...
export default function fffExtension(pi: ExtensionAPI) {
	...
	const resolvedDbPaths = resolveDbPaths({
		frecency: (pi.getFlag("fff-frecency-db") as string | undefined) ?? process.env.FFF_FRECENCY_DB,
		history: (pi.getFlag("fff-history-db") as string | undefined) ?? process.env.FFF_HISTORY_DB,
	});
	...
	function ensureFinder(cwd: string): Promise<FileFinderApi> {
		if (mainFinder && !mainFinder.isDestroyed && finderCwd === cwd) return Promise.resolve(mainFinder);
		...
		try {
			...
			if (entries) {
				const modeEntry = [...entries]
					.reverse()
					.find((e: { type: string; customType?: string }) => e.type === "custom" && e.customType === "fff-mode");
				if (
					modeEntry &&
	...
	if (enableMultiGrep) {
		...
		pi.registerTool({
			...
			async execute(_toolCallId, params, signal) {
				...
				const notices: string[] = [];
				if (result.items.length >= effectiveLimit) notices.push(`${effectiveLimit}+ matches (refine patterns)`);
	...
	pi.registerCommand("fff-mode", {
		...
		handler: async (args, ctx) => {
			...
			const note = (oldMode === "override") !== (newMode === "override") ? " (tool name change requires /reload)" : "";
