...
import type { FileFinderApi } from "@ff-labs/fff-bun";
import type { FilePickerFactory } from "./file-picker.ts";
import { HOME_DIR } from "./paths.ts";
...
export class AuxFinderPool {
	...
	async acquire(maybeRoot: string, opts?: { exact?: boolean }): Promise<{ finder: FileFinderApi; root: string }> {
		this.sweepIdle();
...
export function routePathConstraint(
	...
	if (candidate === "~" || candidate.startsWith("~/")) candidate = path.join(HOME_DIR, candidate.slice(1));
