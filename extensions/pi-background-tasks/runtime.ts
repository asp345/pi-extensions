...
export class BackgroundRuntime {
	...
	start(
		...
		const task: ManagedTask = {
			...
			closed: false,
			stopRequested: false,
			timers: new TaskTimers(),
			timeoutMs,
			waiters: new Set(),
		};
		log.isActive = () => !task.closed && this.tasks.has(task.info.id);
		...
		task.timers.armHeartbeat(task.info.heartbeatMs, () => this.heartbeat(task));
	...
	private armTimeout(task: ManagedTask): void {
		...
		if (timeoutMs === undefined || task.closed) return;
		task.timers.armTimeout(timeoutMs, () => {
	...
	private heartbeat(task: ManagedTask): void {
		...
		this.emit({ type: "running", task: snapshot(task), output: tail(task.log.text, ALERT_LIMIT) });
	}
	...
	private finish(task: ManagedTask, code: number | null): void {
		...
		task.info.updatedAt = Date.now();
		task.timers.clearAll();
		const result: WaitResult = { task: snapshot(task), output: task.log.text };
		for (const waiter of task.waiters) waiter(result);
		task.waiters.clear();
		if (task.info.notify && !this.shuttingDown) {
			this.emit({ type: "exit", task: snapshot(task), output: tail(task.log.text, ALERT_LIMIT) });
			this.reportState();
		}
		if (!this.shuttingDown) this.update();
	}
	...
	waitForExit(
		...
		if (task.closed) return Promise.resolve({ task: snapshot(task), output: task.log.text });
