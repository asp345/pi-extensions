...
- `/agents` or `ctrl+shift+s` opens the dashboard: counts, then `Running`, `Idle`, and `Inactive` sections with `Session`, `Model` (`id:thinking`), `Activity`, `Cost`, and `Age` columns, and the last text or error of the selected subagent. Keys: `↑`/`↓` or `j`/`k` move, `s` stops the selected subagent, `q` or `esc` closes.
